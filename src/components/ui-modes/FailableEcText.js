  ...
  computed: {
    ...
    text() {
      if (this.currentEternityChallengeId === 4) {
        return `${formatInt(this.currentResource)} / ${formatInt(this.maximumResource)} 已使用的无限次数`
      }
      ...
      return `${TimeSpan.fromSeconds(this.currentResource.toNumber()).toString()} /
        ${TimeSpan.fromSeconds(this.maximumResource.toNumber()).toString()} 时间消耗`
    },
  },
  methods: {
    update() {
      if (EternityChallenge.current && [4, 12].includes(EternityChallenge.current.id)) {
        ...
        if (this.currentEternityChallengeId === 4) {
          ...
          this.currentResource = new Decimal(Time.thisEternity.totalSeconds / debugOptions.getGlobalSpeedFactor())
        }
