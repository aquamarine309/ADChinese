  ...
  methods: {
    update() {
      ...
      if (isAbove308) {
        ...
        const timeEstimateText = timeToThousand.eq(0) ? '极长的时间' : `${TimeSpan.fromSeconds(timeToThousand.toNumber())}`
        this.remainingTimeText = `你每秒获得 ${formatX(gainFactorPerSecond, 2, 1)} 复制器` + ` (${timeEstimateText} 后到达 ${format(nextMilestone)})`
      } else {
      ...
      if (this.remainingTimeText === '') {
        if (remainingTime === 0) {
          this.remainingTimeText = `达到无限复制器 (通常需要
          ...
          this.remainingTimeText = `距离无限复制器还有近似 ${TimeSpan.fromSeconds(remainingTime)}`
        } else {
          this.remainingTimeText = `距离无限复制器还有 ${TimeSpan.fromSeconds(remainingTime)}`
        }
      }
