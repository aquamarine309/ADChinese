...
export default {
  ...
  computed: {
    ...
    fullScreenTooltip() {
      return this.fullScreen ? '退出全屏模式' : '打开全屏模式'
    },
    errorTooltip() {
      return `你的脚本有 ${format(this.errorCount)} 个错误。`
    },
    nameTooltip() {
      return this.isNameTooLong ? `脚本名称限制为 ${formatInt(AutomatorData.MAX_ALLOWED_SCRIPT_NAME_LENGTH)} 字符。` : ''
    },
    currentScriptID: {
      get() {
    ...
    importTooltip() {
      return this.canMakeNewScript ? '导入单个自动机脚本或数据' : '脚本数量已达到上限'
    },
