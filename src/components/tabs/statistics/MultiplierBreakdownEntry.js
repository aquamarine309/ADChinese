...
export default {
  ...
  computed: {
    ...
    disabledText() {
      if (!this.resource.isBase) return `总效果失效、无效或降至${formatX(1)}`
  ...
  methods: {
    ...
    calculatePercents() {
      ...
      for (const entry of this.entries) {
        ...
        const perc = entry.data.pow >= 1 ? multFrac / totalPosPow + powFrac * (1 - 1 / totalPosPow) : (Math.log(entry.data.pow) / Math.log(totalNegPow)) * (totalNegPow - 1)
    ...
    styleObject(index) {
      ...
      return {
        ...
        background: isNerf ? `repeating-linear-gradient(-45deg, var(--color-bad), ${iconObj?.color} 0.8rem)` : iconObj?.color,
      }
    },
  ...
  >
    ...
    >
      ...
      >
        <div>
          "基础反物质产量 "是指您在当前反物质升级的情况下生产的反物质数量
          就像你在献祭后等待了一段固定时间（{{ formatInt(10) }}-{{ formatInt(40) }} 秒，取决于你的反物质数量）。
          如果你的反物质已经生产了一段时间，这可能会误导你的实际产量。
          但随着你在游戏中的不断深入，数量会越来越多，相对的不匹配也会越来越小。
        </div>
        <div v-if="inNC12">
          对于某些条目，“普通挑战12”中此选项卡的细目可能不准确，可能会计算适用于所有反物质维度的额外乘数，而不仅仅是显示的那些。
        </div>
      </div>
    </div>
  </div>
