...
export default {
  ...
  computed: {
    ...
    respecTooltip() {
      ...
      return this.respec ? `重置已激活，将在${reset}后将当前装备的符文放入库存。` : `当前装备的符文将在${reset}后保持装备状态。`
    },
    undoTooltip() {
      if (!this.undoSlotsAvailable) return '你没有足够的库存空间来卸载符文'
      return this.undoAvailable ? '卸载最后装备的符文并将现实回滚到装备它的时刻。' + '（大多数资源将被完全重置）' : '仅可撤销在当前现实期间装备的符文'
    },
    unequipText() {
  ...
  >
    ...
    >
      ...
      >
        将符文卸载至：
        <br>
        <span v-if="respecIntoProtected">被保护行</span>
