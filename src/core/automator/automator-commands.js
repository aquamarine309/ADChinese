    ...
    run: () => {
      ...
      if (!evalComparison()) {
        AutomatorData.logCommandEvent(
          `条件 ${parseConditionalIntoText(ctx)} 为${convertBooleanToChinese(isUntil)}，
...
export const AutomatorCommands = [
  {
    ...
    rule: ($) => () => {
      ...
      $.OR([
        { ALT: () => $.CONSUME(T.On) },
        { ALT: () => $.CONSUME(T.Off) },
        { ALT: () => $.OR1([{ ALT: () => $.SUBRULE($.duration) }, { ALT: () => $.SUBRULE($.xHighest) }, { ALT: () => $.SUBRULE($.currencyAmount) }]) },
      ])
    },
    ...
    validate: (ctx, V) => {
      ...
      if (ctx.PrestigeEvent && ctx.currencyAmount) {
        ...
        if (desired$ !== specified$) {
          V.addError(ctx.currencyAmount, `自动机货币不匹配重置层（${desired$} vs ${specified$}）`, `使用 ${desired$} 作为重置层货币`)
          return false
        }
      }
      ...
      if (ctx.PrestigeEvent[0].tokenType === T.Infinity) {
        if (!Autobuyer.bigCrunch.isUnlocked) {
          V.addError(ctx.PrestigeEvent, '未解锁自动无限', '完成挑战 12后解锁这条指令')
          return false
        }
        if (advSetting && !EternityMilestone.bigCrunchModes.isReached) {
          V.addError((ctx.duration || ctx.xHighest)[0], '未解锁自动大坍缩的高级选项', `永恒次数达到 ${formatInt(EternityMilestone.bigCrunchModes.config.eternities)} 以使用该指令`)
          return false
        }
      }
      if (ctx.PrestigeEvent[0].tokenType === T.Eternity) {
        if (!EternityMilestone.autobuyerEternity.isReached) {
          V.addError(ctx.PrestigeEvent, '未解锁自动永恒', `永恒次数达到 ${formatInt(EternityMilestone.autobuyerEternity.config.eternities)} 以使用该指令`)
          return false
        }
        if (advSetting && !RealityUpgrade(13).isBought) {
          V.addError((ctx.duration || ctx.xHighest)[0], '未解锁自动永恒的高级选项', '购买对应的现实升级，解锁更多的自动永恒选项')
          return false
        }
      }
      if (ctx.PrestigeEvent[0].tokenType === T.Reality) {
        if (!RealityUpgrade(25).isBought) {
          V.addError(ctx.PrestigeEvent, '未解锁自动现实', '购买对应的现实升级，解锁自动现实')
          return false
        }
        if (advSetting) {
          V.addError((ctx.duration || ctx.xHighest)[0], '自动现实不能设置为每隔 X 秒自动进行或最高现实机器的 X 倍', '使用获得现实机器的数量进行自动现实')
          return false
        }
      }
    ...
    compile: (ctx) => {
      ...
      return () => {
        ...
        AutomatorData.logCommandEvent(
          `自动${ctx.PrestigeEvent[0].image}
          切换至${autobuyer.isActive ? '开启' : '关闭'}状态${settingString}`,
          ctx.startLine
        )
    ...
    compile: (ctx) => {
      ...
      return (S) => {
        ...
        if (S.commandState === null) {
          ...
          S.commandState.timeMs += Math.max(Time.unscaledDeltaTime._ms / debugOptions.getGlobalSpeedFactor(), AutomatorBackend.currentInterval)
        }
  ...
  },
  {
    ...
    compile: (ctx) => {
      ...
      return () => {
        ...
        AutomatorData.logCommandEvent(`${prestigeName} triggered (${findLastPrestigeRecord(prestigeName)})`, ctx.startLine)
    ...
    validate: (ctx, V) => {
      ...
      if (ctx.Id) {
        ...
        if (!split || ctx.Id[0].isInsertedInRecovery) {
          V.addError(ctx, '预设序号不存在', '查看时间研究页面，输入正确的时间研究预设序号')
          return false
        }

        const id = parseInt(split[1], 10)
        if (id < 1 || id > 6) {
          V.addError(ctx.Id[0], `找不到序号为 ${id} 的时间研究预设`, '输入有效的时间研究预设序号 （1 – 6）')
          return false
        }
        ctx.$presetIndex = id
        return true
      ...
      if (ctx.Name) {
        ...
        if (!split || ctx.Name[0].isInsertedInRecovery) {
          V.addError(ctx, '预设名称不存在', '查看时间研究页面，输入正确的时间研究预设名称')
          return false
        }
        ...
        if (presetIndex === 0) {
          V.addError(ctx.Name[0], `找不到名称为 [${split[1]}] 的时间研究预设 （注意：名称区分大小写）`, '确认你输入了正确的时间研究预设名称')
          return false
        }
  ...
  },
  {
    ...
    compile: (ctx) => {
      ...
      return () => {
        ...
        if (nowait) {
          AutomatorData.logCommandEvent(`因出现 NOWAIT, 且未满足解锁时间膨胀的要求，跳过解锁时间膨胀的指令`, ctx.startLine)
          return AUTOMATOR_COMMAND_STATUS.NEXT_INSTRUCTION
        }
        return AUTOMATOR_COMMAND_STATUS.NEXT_TICK_SAME_INSTRUCTION
      }
      ...
      return {
        run: (S) => {
          ...
          if (S.commandState.prestigeLevel >= prestigeLevel) {
            AutomatorData.logCommandEvent(`已进行 ${prestigeName}，退出 until 循环`, ctx.startLine)
            return AUTOMATOR_COMMAND_STATUS.NEXT_INSTRUCTION
          }
          AutomatorBackend.push(commands)
          AutomatorData.logCommandEvent(`未进行${prestigeName}，跳转至第 ${AutomatorBackend.translateLineNumber(ctx.LCurly[0].startLine + 1)} 行（until 循环的开始）`, ctx.startLine)
          return AUTOMATOR_COMMAND_STATUS.SAME_INSTRUCTION
        },
        blockCommands: commands,
      }
    ...
    compile: (ctx, C) => () => {
      ...
      if (doneWaiting) {
        ...
        if (AutomatorData.isWaiting) {
          ...
          AutomatorData.logCommandEvent(`已跳过 WAIT 指令（${parseConditionalIntoText(ctx)} 为真）`, ctx.startLine)
        }
  ...
  },
  {
    ...
    compile: (ctx) => () => {
      ...
      if (bhCond) {
        const timeWaited = TimeSpan.fromMilliseconds(Date.now() - AutomatorData.waitStart).toStringShort()
        AutomatorData.logCommandEvent(`已执行 WAIT 指令，继续运行（等待 ${timeWaited} 后${bhStr}）`, ctx.startLine)
        AutomatorData.isWaiting = false
        return AUTOMATOR_COMMAND_STATUS.NEXT_INSTRUCTION
      }
      if (!AutomatorData.isWaiting) {
