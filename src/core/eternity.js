...
function giveEternityRewards(auto) {
  player.records.bestEternity.time = Math.min(player.records.thisEternity.time / debugOptions.getGlobalSpeedFactor(), player.records.bestEternity.time)
  Currency.eternityPoints.add(gainedEternityPoints())

  const newEternities = gainedEternities().times(
...
export function animateAndEternity(callback) {
  ...
  const hasAnimation =
    ...
    ((player.dilation.active && player.options.animations.dilation) || (!player.dilation.active && player.options.animations.eternity))
...
export function gainedEternities() {
  return Pelle.isDisabled('eternityMults')
    ? new Decimal(1)
    : new Decimal(getAdjustedGlyphEffect('timeetermult')).timesEffectsOf(RealityUpgrade(3), Achievement(113)).pow(AlchemyResource.eternity.effectValue).plus(debugOptions.getEternityCountMultiplier())
}
