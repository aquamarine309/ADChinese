...
export class EternityChallengeState extends GameMechanicState {
  ...
  fail(auto = false) {
    ...
    if (auto) {
      if (this.id === 4) {
        reason = restriction => `自动完成永恒挑战功能完成了` +
        `永恒挑战 ${this.id} 后，你的无限次数超过了下一级的` +
        `限制（${formatInt(restriction)} 或更少的永恒次数）`;
      } else if (this.id === 12) {
        reason = restriction => `自动完成永恒挑战功能完成了` +
        `永恒挑战 ${this.id} 后，你在本次永恒所花费的时间超过了下一级的` +
        `限制（${format(restriction, 0, 1)}）`;
      }
    } else if (this.id === 4) {
      reason = restriction => `由于你的无限次数超过了 ${formatInt(restriction)}，永恒挑战 ${this.id} 失败`;
    } else if (this.id === 12) {
      reason = restriction => `由于你在本次永恒所花费的时间超过了 ${format(restriction, 0, 1)}，永恒挑战 ${this.id} 失败`;
    }
