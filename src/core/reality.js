...
export function simulatedRealityCount(advancePartSimCounters) {
  ...
  const simCount = (multiversalSim + 1) * (amplifiedSim + 1) + player.partSimulatedReality - 1 + debugOptions.getAdditionalRealityAmplify()
...
export function requestManualReality() {
  ...
  if (GameCache.glyphInventorySpace.value === 0) {
    Modal.message.show('没空位了，Shift点击一些符文来摆脱它们', { closeEvent: GAME_EVENT.GLYPHS_CHANGED })
    return
  }
...
function updateRealityRecords(realityProps) {
  ...
  player.records.bestReality.time = Math.min(player.records.thisReality.time / debugOptions.getGlobalSpeedFactor(), player.records.bestReality.time)
...
function giveRealityRewards(realityProps) {
  ...
  Currency.realities.add(realityAndPPMultiplier * debugOptions.getRealityPerkCountMultiplier())
  Currency.perkPoints.add(realityAndPPMultiplier * debugOptions.getRealityPerkCountMultiplier())
  ...
  if (Teresa.isRunning) {
    ...
    const modalText = `你已完成了特蕾莎的现实！${
      isHigher ? `由于你获得了更多反物质，你的符文献祭乘数从 ${format(current, 2, 2)} 提升至 ${format(newMultiplier, 2, 2)}` : `你在本次运行中未获得更多反物质，因此特蕾莎的符文献祭乘数未提升`
    }。`
...
export function beginProcessReality(realityProps) {
  ...
  if (glyphsToProcess < 100) {
    for (let glyphNum = 0; glyphNum < glyphsToProcess; glyphNum++) {
      ...
      if (EffarigUnlock.glyphFilter.isUnlocked) {
        const glyphChoices = GlyphSelection.glyphList(GlyphSelection.choiceCount, realityProps.gainedGlyphLevel, { rng })
