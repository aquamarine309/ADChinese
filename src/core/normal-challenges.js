...
export function updateNormalAndInfinityChallenges(diff) {
  if (NormalChallenge(11).isRunning || InfinityChallenge(6).isRunning) {
    if (AntimatterDimension(2).amount.neq(0)) {
      ...
      Currency.matter.multiply(Decimal.pow(cappedBase, diff / debugOptions.getGlobalSpeedFactor() / 20))
    }
...
class NormalChallengeState extends GameMechanicState {
  ...
  updateChallengeTime() {
    ...
    if (bestTimes[this.id - 2] <= player.records.thisInfinity.time / debugOptions.getGlobalSpeedFactor()) {
      return
    }
    player.challenge.normal.bestTimes[this.id - 2] = player.records.thisInfinity.time / debugOptions.getGlobalSpeedFactor()
