...
export const AD = {
  total: {
    name: (dim) => {
      if (dim) return `反物质维度 ${dim} 倍数`
      if (NormalChallenge(12).isRunning) {
        if (MultiplierTabHelper.actualNC12Production().eq(0)) return '所有维度的基础反物质生产'
        return `来自${MultiplierTabHelper.isNC12ProducingEven() ? '偶数' : '奇数'}维度的基础反物质生产`
      }
      return '基础反物质生产'
    },
    displayOverride: (dim) => {
  ...
  dimboost: {
    name: (dim) => (dim ? `维度提升对反物质维度 ${dim}` : '维度提升'),
    multValue: (dim) =>
      dim
  ...
  infinityUpgrade: {
    name: (dim) => (dim ? `无限升级（反物质维度 ${dim})` : '无限升级'),
    multValue: (dim) => {
  ...
  infinityChallenge: {
    name: (dim) => (dim ? `无限挑战（反物质维度 ${dim})` : '无限挑战'),
    multValue: (dim) => {
  ...
  timeStudy: {
    name: (dim) => (dim ? `时间研究（反物质维度 ${dim})` : '时间研究'),
    multValue: (dim) => {
  ...
  effectNC: {
    name: (dim) => (dim ? `普通挑战效果（反物质维度 ${dim})` : '普通挑战效果'),
  ...
  nerfIC: {
    name: (dim) => (dim ? `无限挑战削弱（反物质维度 ${dim})` : '无限挑战削弱'),
    multValue: (dim) => {
      let dimMults = Array.repeat(DC.D1, 9)
