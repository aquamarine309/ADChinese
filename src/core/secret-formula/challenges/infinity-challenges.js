...
export const infinityChallenges = [
  ...
  {
    id: 3,
    description: () => `计数频率的升级总是增加 ${formatX(1)}. 但你每次购买计数频率升级时，你的反物质维度会获得一个基于反物质星系数量的倍数加成。`,
    goal: DC.E5000,
    isQuickResettable: false,
  ...
  {
    id: 4,
    description: () => `只有最近一次购买的反物质维度可以正常生产。所有其他的反物质维度的产量将下降（${formatPow(0.25, 2, 2)}）。`,
    goal: DC.E13000,
    isQuickResettable: true,
    effect: 0.25,
    reward: {
  ...
  {
    id: 5,
    description: `购买第 1-4 反物质维度时，会增加所有价格较小或相等的反物质维度价格。购买第 5-8 反物质维度时，会增加所有价格较大或相等的反物质维度价格。`,
    goal: DC.E16500,
    isQuickResettable: true,
    reward: {
      description: () => `星系的加成提升${formatPercents(0.1)}. 维度提升和反物质星系的需求减少${formatInt(1)}。`,
      effect: 1.1,
    },
    unlockAM: DC.E18000,
  },
  {
    id: 6,
    description: () => `一旦你拥有了一个第二反物质维度，反物质维度的倍数加成就会被除以指数级增加的物质数量。`,
    goal: DC.D2E22222,
    isQuickResettable: true,
  ...
  {
    id: 8,
    description: () => `购买任何东西后，你的产量会恢复为100％，并从此开始迅速下降。`,
    goal: DC.E27000,
    isQuickResettable: true,
    effect: () => DC.D0_8446303389034288.pow(Math.max(0, player.records.thisInfinity.time - player.records.thisInfinity.lastBuyTime) / debugOptions.getGlobalSpeedFactor()),
    reward: {
      description: '基于第一和第八维度的倍数，获得第二至第七维度额外倍数',
