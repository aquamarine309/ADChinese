...
export const eternityChallenges = [
  ...
  },
  {
    ...
    formatRestriction: restriction => (restriction === 0
      ? "没有任何无限次数"
      : `在 ${formatInt(restriction)} 无限次数以内`),
    ...
    reward: {
      ...
      formatEffect: value => {
        ...
        return TimeStudy(31).canBeApplied
          ? `${formatX(value.pow(1 / TimeStudy(31).effectValue), 2, 1)} （时间研究 31 之后：${mult}）`
          : mult;
      }
    }
  },
  ...
  },
  {
    ...
    description: () => (PlayerProgress.realityUnlocked()
      ? `游戏速度放慢 ${formatInt(1000)} 倍，并禁用其它影响游戏速度的机制。${specialInfinityGlyphDisabledEffectText()}`
