...
export const BlackHoles = {
  ...
  togglePause: (automatic = false) => {
    ...
    GameUI.notify.blackHole(`${blackHoleString} 已${automaticString}${pauseType}`)
  },

  get unpauseAccelerationFactor() {
    if (this.arePermanent) return 1
    return Math.clamp(((player.records.realTimePlayed - player.blackHolePauseTime) * debugOptions.getGlobalSpeedFactor()) / (1000 * this.ACCELERATION_TIME), 0, 1)
  },
