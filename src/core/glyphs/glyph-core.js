...
export const Glyphs = {
  ...
  get activeSlotCount() {
    ...
    return 3 + Effects.sum(RealityUpgrade(9), RealityUpgrade(24)) + debugOptions.getAdditionalGlyphSlotCount()
  },
  get protectedSlots() {
  ...
  equip(glyph, targetSlot) {
    ...
    if (this.active[targetSlot] === null) {
      if (sameSpecialTypeIndex >= 0) {
        Modal.message.show(`You may only have one ${glyph.type.capitalize()} Glyph equipped!`, { closeEvent: GAME_EVENT.GLYPHS_CHANGED })
        return
      }
  ...
  unequipAll(forceToUnprotected = false) {
    ...
    if (stillEquipped && !fastReality) {
      ...
      setTimeout(
        () =>
          Modal.message.show(
            `因仓库空间不足，无法卸下部分符文。 清理${target}的空间${hasOther ? '或选择保留的地方' : ''}
        以卸下符文.`,
  ...
  isObjectivelyUseless(glyph, threshold, inventoryIn) {
    ...
    function hasSomeBetterEffects(glyphA, glyphB, comparedEffects) {
      for (const effect of comparedEffects) {
        const c = effect.compareValues(effect.effect(glyphA.level, glyphA.strength), effect.effect(glyphB.level, glyphB.strength))
    ...
    const toCompare = (inventoryIn ?? this.inventory)
      .concat(this.active)
      .filter((g) => g !== null && g.type === glyph.type && g.id !== glyph.id && (g.level >= glyph.level || g.strength >= glyph.strength) && (g.effects & glyph.effects) === glyph.effects)
  ...
  giveCursedGlyph() {
    if (GameCache.glyphInventorySpace.value === 0) {
      Modal.message.show('仓库已满，请献祭一些符文留出空位', { closeEvent: GAME_EVENT.GLYPHS_CHANGED })
      return
    }
    const cursedCount = this.allGlyphs.filter((g) => g !== null && g.type === 'cursed').length
    if (cursedCount >= 5) {
      GameUI.notify.error(`你不需要超过 ${format(5)} 个诅咒符文!`)
    } else {
      this.addToInventory(GlyphGenerator.cursedGlyph())
      GameUI.notify.error('诅咒符文已被创造...')
    }
  },
}
...
export function respecGlyphs() {
  if (!Glyphs.unequipAll()) {
    Modal.message.show('Some of your Glyphs could not be unequipped due to lack of inventory space.', { closeEvent: GAME_EVENT.GLYPHS_CHANGED })
  }
