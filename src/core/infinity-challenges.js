...
class InfinityChallengeState extends GameMechanicState {
  ...
  updateChallengeTime() {
    ...
    if (bestTimes[this.id - 1] <= player.records.thisInfinity.time / debugOptions.getGlobalSpeedFactor()) {
      return
    }
    player.challenge.infinity.bestTimes[this.id - 1] = player.records.thisInfinity.time / debugOptions.getGlobalSpeedFactor()
