...
function totalEPMult() {
  return Pelle.isDisabled('EPMults')
    ...
    : getAdjustedGlyphEffect('cursedEP')
        ...
        .timesEffectsOf(EternityUpgrade.epMult, TimeStudy(61), TimeStudy(122), TimeStudy(121), TimeStudy(123), RealityUpgrade(12), GlyphEffect.epMult)
}
...
export function addInfinityTime(time, realTime, ip, infinities) {
  ...
  player.records.recentInfinities.unshift([time / debugOptions.getGlobalSpeedFactor(), realTime, ip, infinities, challenge])
  GameCache.bestRunIPPM.invalidate()
}

export function resetInfinityRuns() {
  player.records.recentInfinities = Array.from({ length: 10 }, () => [Number.MAX_VALUE, Number.MAX_VALUE, DC.D1, DC.D1, ''])
  GameCache.bestRunIPPM.invalidate()
}
...
export function addEternityTime(time, realTime, ep, eternities) {
  ...
  player.records.recentEternities.unshift([time / debugOptions.getGlobalSpeedFactor(), realTime, ep, eternities, challenge, gainedTP])
  GameCache.averageRealTimePerEternity.invalidate()
}

export function resetEternityRuns() {
  player.records.recentEternities = Array.from({ length: 10 }, () => [Number.MAX_VALUE, Number.MAX_VALUE, DC.D1, DC.D1, '', DC.D0])
  GameCache.averageRealTimePerEternity.invalidate()
}
...
export function addRealityTime(time, realTime, rm, level, realities, ampFactor, projIM) {
  ...
  player.records.recentRealities.unshift([time / debugOptions.getGlobalSpeedFactor(), realTime, rm.times(ampFactor), realities, reality, level, shards * ampFactor, projIM])
}
...
export function realTimeMechanics(realDiff) {
  // realDiff have calculated debugOptions.getGlobalSpeedFactor() effect.
  ...
  Ra.memoryTick(realDiff / Math.pow(debugOptions.getGlobalSpeedFactor(), 1 / 2), !Enslaved.isStoringRealTime)
  if (AlchemyResource.momentum.isUnlocked) {
    player.celestials.ra.momentumTime += realDiff * Achievement(175).effectOrDefault(1)
  }

  DarkMatterDimensions.tick(realDiff / Math.pow(debugOptions.getGlobalSpeedFactor(), 1 / 2))

  // When storing real time, skip everything else having to do with production once stats are updated
  if (Enslaved.isStoringRealTime) {
    // Time recording should not be speeded up by debugOptions.getGlobalSpeedFactor().
    realDiff /= debugOptions.getGlobalSpeedFactor()
...
export function gameLoop(passDiff, options = {}) {
  ...
  const realDiff = (diff === undefined ? Math.clamp(thisUpdate - player.lastUpdate, 1, 8.64e7) : diff) * debugOptions.getGlobalSpeedFactor()
  if (!GameStorage.ignoreBackupTimer) player.backupTimer += realDiff / debugOptions.getGlobalSpeedFactor()
  ...
  if (player.options.hibernationCatchup && passDiff === undefined && realDiff > 6e4 * debugOptions.getGlobalSpeedFactor()) {
    GameIntervals.gameLoop.stop()
    simulateTime(realDiff / 1000 / debugOptions.getGlobalSpeedFactor(), true)
  ...
  } else diff *= debugOptions.getGlobalSpeedFactor()
  ...
  const blackHoleDiff = realDiff / Math.sqrt(debugOptions.getGlobalSpeedFactor())
  ...
  if (!Achievement(188).isUnlocked) {
    player.records.realTimeDoomed += realDiff / debugOptions.getGlobalSpeedFactor()
    player.records.realTimePlayed += realDiff / debugOptions.getGlobalSpeedFactor()
    player.records.totalTimePlayed += diff
    player.records.thisInfinity.realTime += realDiff / debugOptions.getGlobalSpeedFactor()
    player.records.thisInfinity.time += diff
    player.records.thisEternity.realTime += realDiff / debugOptions.getGlobalSpeedFactor()
    ...
    player.records.thisReality.realTime += realDiff / debugOptions.getGlobalSpeedFactor()
  ...
  applyAutoprestige(realDiff / debugOptions.getGlobalSpeedFactor())
  ...
  laitelaRealityTick(realDiff / debugOptions.getGlobalSpeedFactor())
  Achievements.autoAchieveUpdate(diff)
  V.checkForUnlocks()
  AutomatorBackend.update(realDiff / debugOptions.getGlobalSpeedFactor())
  Pelle.gameLoop(realDiff)
  GalaxyGenerator.loop(realDiff)
  GameEnd.gameLoop(realDiff / debugOptions.getGlobalSpeedFactor())
...
function passivePrestigeGen() {
  ...
  if (!EternityChallenge(4).isRunning) {
    ...
    if (EffarigUnlock.eternity.isUnlocked) {
      ...
      infGen = infGen.plus(
        gainedInfinities()
          .times(Currency.eternities.value.minus(eternitiedGain.div(2).floor()))
...
function laitelaRealityTick(realDiff) {
  ...
  if (laitelaInfo.entropy >= 1) {
    let completionText = `莱特拉的现实已于 ${Time.thisRealityRealTime.toStringShort()} 后失稳`
    ...
    if (Laitela.realityReward > oldInfo.realityReward) {
      completionText += `<br><br>暗物质乘数: ${formatX(oldInfo.realityReward, 2, 2)}
      ...
      if (oldInfo.fastestCompletion === 3600 || (oldInfo.fastestCompletion === 300 && oldInfo.difficultyTier > 0)) {
        if (Time.thisRealityRealTime.totalSeconds < 30) {
          ...
          completionText += `<br>最佳完成时间: 无 ➜ 失稳
          <br>最高可用维度: ${formatInt(8 - oldInfo.difficultyTier)} ➜
          ${formatInt(8 - laitelaInfo.difficultyTier)}`
        } else {
          // First attempt - not destabilising
          completionText += `<br>最佳完成时间: 无 ➜
            ${TimeSpan.fromSeconds(laitelaInfo.fastestCompletion).toStringShort()}
            <br>最高可用维度: ${formatInt(8 - laitelaInfo.difficultyTier)}`
        }
      } else if (Time.thisRealityRealTime.totalSeconds < 30) {
        // Second+ attempt - destabilising
        completionText += `<br>最佳完成时间:  ${TimeSpan.fromSeconds(oldInfo.fastestCompletion).toStringShort()}
          ➜ 失稳
          <br>最高可用维度: ${formatInt(8 - oldInfo.difficultyTier)} ➜
          ${formatInt(8 - laitelaInfo.difficultyTier)}`
      } else {
      ...
      completionText += `你需要用快于
        ${TimeSpan.fromSeconds(laitelaInfo.fastestCompletion).toStringShort()} 的时间让现实失稳来改进乘数。`
    }
...
function laitelaBeatText(disabledDim) {
  switch (disabledDim) {
    case 1:
      return `<br><br>莱特拉的现实现在将完全禁用所有维度的生产。
        仍然可以进入现实，但进一步的破坏不再可能。
        为了完全破坏现实，你还将获得额外的 ${formatX(8)} 暗能量增益。`
    case 2:
      return `<br><br>莱特拉的现实现在将在未来的运行中禁用所有第 2 维度的生产，
        但奖励将比之前强 ${formatInt(100)} 倍。完全破坏最后一个维度的现实将为你提供额外的 ${formatX(8)} 暗能量增益。`
    case 3:
      return `<br><br>莱特拉的现实现在将在未来的运行中禁用所有第 3 维度的生产，
        但奖励将比之前强 ${formatInt(100)} 倍。`
    case 8:
      return `<br><br>莱特拉的现实现在将在未来的运行中禁用所有第 8 维度的生产，
        但奖励将比之前强 ${formatInt(100)} 倍。通过再次在 ${formatInt(30)} 秒内达到破坏，可以为每个剩余的维度重复此增益。`
    default:
      return `<br><br>莱特拉的现实现在将在未来的运行中禁用所有第 ${disabledDim} 维度的生产，
        但奖励将比之前强 ${formatInt(100)} 倍。`
  }
}
...
export function getTTPerSecond() {
  ...
  // Glyph TT generation
  const glyphTT = Teresa.isRunning || Enslaved.isRunning || Pelle.isDoomed ? 0 : getAdjustedGlyphEffect('dilationTTgen') * ttMult

  // Dilation TT generation
  const dilationTT = DilationUpgrade.ttGenerator.isBought ? DilationUpgrade.ttGenerator.effectValue.times(Pelle.isDoomed ? 1 : ttMult) : DC.D0
...
export function simulateTime(seconds, real, fast) {
  seconds *= debugOptions.getGlobalSpeedFactor()
  ...
  if (seconds < 0) return
  let ticks = Math.floor((seconds / debugOptions.getGlobalSpeedFactor()) * 20)
  GameUI.notify.showBlackHoles = false

  // Limit the tick count (this also applies if the black hole is unlocked)
  const maxTicks = GameStorage.maxOfflineTicks((1000 * seconds) / debugOptions.getGlobalSpeedFactor(), GameStorage.offlineTicks ?? player.options.offlineTicks)
  ...
  let remainingRealSeconds = seconds / debugOptions.getGlobalSpeedFactor()
    ...
    Async.run(loopFn, ticks, {
      ...
      asyncEntry: (doneSoFar) => {
        ...
        ui.$viewModel.modal.progressBar = {
          label: '离线进度模拟',
          info: () => `游戏正在以较低的精度运行，以便快速计算您离开时应获得的资源。
              有关技术细节，请参阅如何玩 “离线进度” 条目。
              如果您失去耐心，想尽快回到游戏中，可以点击 “加速” 按钮来使用一半的时间来模拟剩余的（最小为 ${formatInt(500)} 离线时间刻。
              点击 “跳过” 按钮则会使用所有剩余离线时间于 ${formatInt(10)} 游戏时间刻。`,
          progressName: 'Ticks',
          current: doneSoFar,
          max: ticks,
          startTime: Date.now(),
